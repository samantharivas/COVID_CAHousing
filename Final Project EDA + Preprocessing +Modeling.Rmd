...
```{r}
CA.ts <- ts(extracted_CA$CA, frequency = 12, start = c(1990, 1), end = c(2023, 9))
```
...
par(mfrow = c(2,1))
plot(CA.ts, xlab = "Time", ylab = "CA", ylim = c(160000 , 900000), bty = "l")
...
# Training set is from January 1990 - December 2019. Validation set is from January 2020 - September 2023

```{r}
train <- window(CA.ts, start = c(2014, 1), end = c(2018, 12))
validation <- window(CA.ts, start = c(2019, 1))
```
...
```{r}
st_forecast <- forecast(st_model, h=45)
st_forecast
```

# plot the Season and Trend Linear Model

```{r}
autoplot(train) + 
  autolayer(st_forecast, color = "red", series = "Time Series Linear Model") +
   autolayer(validation, color = "blue", series = "Actual")
...
```
Based on RMSE, the Naive Model performed the best 
...
```{r}
extracted_SF <- Cali[, c("Mon.Yr", "San.Francisco")]
extracted_SF
```
...
```{r}
SF.ts <- ts(extracted_SF$San.Francisco, frequency = 12, start = c(1990, 1), end = c(2023, 9))
```
...
```{r}
plot(SF.ts, xlab = "Mon-Yr", ylab = "SF Home Prices", ylim = c(240000, 2060000), bty = "l")
```
...
par(mfrow = c(2,1))
plot(SF.ts, xlab = "Time", ylab = "SF", ylim = c(240000, 2060000), bty = "l")
lines(home_prices.lm$fitted, lwd = 2)
SF.ts.zoom <- window(SF.ts, start = c(1990, 1), end = c(2023, 9))
plot(SF.ts.zoom, xlab = "Mon-Yr", ylab = "SF", ylim = c(240000, 2060000), bty = "l")
```


# Choosing a timeframe of 2010 - 2023 

# Create 2 Partitions

# Training set is from January 1990 - December 2019. Validation set is from January 2020 - September 2023

# timeseries object for the extracted CA column between January 2010 - September 2023 



```{r}
ts_subset <- window(CA.ts, start = c(2014, 1), end = c(2023, 9))
ts_subset
```
```{r}
plot(ts_subset)
...
# Partition Data
# Training from January 2010 - December 2019 
# Validation January 2020 - September 2023 (45 Months)
```{r}
train2 <- window(ts_subset, start = c(2010, 1), end = c(2019, 12))
validation2 <- window(ts_subset, start = c(2020, 1))
```
...
```{r}
autoplot(ts_subset) +
  labs(title = "CA home prices between 2010 - 2023",
       x = "Time",
       y = "Sales Volume") + 
       theme_minimal()
...
```{r}
plot(ts_subset, 
     main = "California Home Price Time Series 2010 - 2023",
     xlab = "Year",
     ylab = "CA Home Prices",
     type = "l", 
     col = "blue")
...
```{r}
autoplot(train2) + 
  autolayer(validation2, color = "blue")
```
...
```{r}
autoplot(train2) + 
  autolayer(n_model2, color = "red", series = "Naive2") +
   autolayer(validation2, color = "blue", series = "Actual")

```



# Seasonal Naive Model2

```{r}
sn_model2 <- snaive(train2, h = 45)
```
...
```{r}
autoplot(train2) + 
  autolayer(sn_model2, color = "red", series = "Seasonal Naive") +
   autolayer(validation2, color = "blue", series = "Actual")
...
# Season and Trend2  Model

```{r}
st_model2 <- tslm(train2 ~ trend + season)
...
```{r}
st_forecast2 <- forecast(st_model2, h=45)
st_forecast2
```
...
```{r}
autoplot(train) + 
  autolayer(st_forecast2, color = "red", series = "Time Series Linear Model") +
   autolayer(validation2, color = "blue", series = "Actual")
...
accuracy(sn_model2)
accuracy(st_forecast)
accuracy(st_forecast2)

```

The original models were better with the 1990 - 2020 training data
...
# plot linear model 
plot(CA_ts, xlab = "Time", ylab = "CA", ylim = c(160000 , 900000), bty = "l")
lines(home_prices.lm$fitted, lwd = 2)
